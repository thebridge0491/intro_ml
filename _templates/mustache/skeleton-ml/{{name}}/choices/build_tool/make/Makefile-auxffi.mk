...
ML_CCOpt := $(ML_CCOpt) -ccopt -Wall -ccopt -pedantic -ccopt -m64 -ccopt -fPIC -ccopt -std=c99 -ccopt -I$(ffi_incdir)
...
install_files := $(install_files) # build/src/$(parent)/*$(proj)_stubs.* # build/src/swig/*.cmi build/src/swig/*.cmo build/src/swig/*.cma
optional_files := $(optional_files) # build/src/swig/*.cmi build/src/swig/*.cmo build/src/swig/*.cma
...
prep_swig src/$(parent)/classic_c_wrap.c: ## prepare Swig files
	-mkdir -p build src/swig
#	-(cd build ; swig -ocaml -co carray.i)
